...
instructions1 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2 - 100),
                     'Welcome to Lander. Your mission is to land on the color indicated in the top right corner.'
                     )
instructions1.draw(win)
instructions2 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2 - 50),
                     'Left and right arrows control your lateral movement. Space bar fires your main thruster which slows your decent'
                     )
instructions2.draw(win)
instructions3 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2),
                     'You must land with no lateral movement and a rate of decent less then 10'
                     )
instructions3.draw(win)
instructions4 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2 + 50),
                     'You will also crash if you hit the sides or top of the screen.'
                     )
instructions4.draw(win)
instructions5 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2 + 100),
...
while playAgain == 1:
    ...
    for i in range(WIN_WIDTH // PAD_WIDTH):
        landingPadList.append(Line(Point(i * 100, WIN_HEIGHT), Point(i
                              * 100 + 100, WIN_HEIGHT)))
    ...
    target01 = Rectangle(Point(WIN_WIDTH - 100, 0), Point(WIN_WIDTH,
                         100))
    ...
    while True:
        ...
        bottomEdge = lander01.getAnchor().getY() + lander01.getHeight() \
            / 2
        leftEdge = lander01.getAnchor().getX() - lander01.getWidth() / 2
        rightEdge = lander01.getAnchor().getX() + lander01.getWidth() \
            / 2
        ...
        if bottomEdge >= WIN_HEIGHT - 10 and lateralSpeed == 0 \
            and fallingSpeed < 1 and lander01.getAnchor().getX() \
            > randomPad * PAD_WIDTH and lander01.getAnchor().getX() \
            < randomPad * PAD_WIDTH + PAD_WIDTH:
            winner = 1
            break

        # if the and edge of the lander touches the edges then the lose condition is met

        if bottomEdge >= WIN_HEIGHT - 10 or topEdge <= 0 or leftEdge \
            <= 0 or rightEdge >= WIN_WIDTH:
            loser = 1
            break
        ...
        if key == 'space' and fuel != 0:
            ...
            try:
                ...
                flame.undraw()
            flame = Image(Point(lander01.getAnchor().getX(), bottomEdge
                          + 50), 'thrustDown.gif')
            flame.draw(win)
        ...
        if key == 'Left' and fuel != 0:
            ...
            try:
                ...
                flame.undraw()
            flame = Image(Point(rightEdge + 50,
                          lander01.getAnchor().getY()),
                          'thrustRight.gif')
            flame.draw(win)
            flameCounter = UPDATE_RATE // 2
        if key == 'Right' and fuel != 0:
            ...
            try:
                ...
                flame.undraw()
            flame = Image(Point(leftEdge - 50,
                          lander01.getAnchor().getY()), 'thrustLeft.gif'
                          )
            flame.draw(win)
            flameCounter = UPDATE_RATE // 2
    ...
    if loser == 1:
        ...
        lander01.undraw()
        lander01 = Image(Point(lander01.getAnchor().getX(),
                         lander01.getAnchor().getY()), 'boom.gif')
        lander01.draw(win)

        # display the results

        popup1 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2),
                      'YOU DIED IN THE COLD VACCUM OF SPACE, GAME OVER')
        popup1.draw(win)
        popup2 = Text(Point(WIN_WIDTH // 2, WIN_HEIGHT // 2 + 100),
